...
export async function loadFromDisk(): Promise<number> {
  try {
    ...
    const configs: CronJobConfig[] = JSON.parse(raw);
    for (const config of configs) {
      scheduleJob(config);
    }
    // Sync all loaded configs to TrueNorth so admin UI is up to date
    if (configs.length > 0) {
      syncConfigsToTrueNorth().catch((err) =>
        console.error("[cron] Startup config sync failed:", err instanceof Error ? err.message : err)
      );
    }
...
function scheduleJob(config: CronJobConfig): void {
...
async function executeCronJob(config: CronJobConfig): Promise<void> {
 ...
 */
async function syncConfigsToTrueNorth(): Promise<void> {
  if (!TRUENORTH_URL || !HERMES_API_SECRET) return;

  // Group jobs by org
  const byOrg = new Map<string, CronJobConfig[]>();
...
export function updateJob(id: string, updates: Partial<CronJobConfig>): void {
...
export function listJobs(): CronJobConfig[] {
