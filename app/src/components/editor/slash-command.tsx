...
export const defaultSlashCommands: SlashCommandItem[] = [
  ...
  // ============================================================
  {
    ...
    searchTerms: ["video", "youtube", "vimeo", "embed", "media"],
    command: ({ editor, range }) => {
      editor.chain().focus().deleteRange(range).run();
      ...
      if (url) {
        ...
        if (!success) {
          alert("Could not parse video URL. Supported: YouTube, Vimeo.");
        }
      }
    },
  },
  {
    ...
    searchTerms: ["tweet", "twitter", "x", "social", "post"],
    command: ({ editor, range }) => {
      editor.chain().focus().deleteRange(range).run();
      const url = prompt("Enter tweet URL (e.g., https://x.com/user/status/123):");
      if (url) {
        const success = editor.chain().focus().setTweetEmbed({ url }).run();
        if (!success) {
          alert("Could not parse tweet URL. Use format: https://x.com/user/status/ID");
        }
      }
    },
  },
