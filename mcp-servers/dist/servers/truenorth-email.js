...
function getKitConfig() {
    ...
    if (!apiKey)
        throw new Error("Missing CONVERTKIT_API_KEY env var");
    return { apiKey };
}
...
// ── Tools ──────────────────────────────────────────────────────────
server.tool("get_subscriber_overview", "Get high-level subscriber stats: total, active, cancelled, recent growth", {}, async () => {
    try {
        const [total, active, cancelled, new7d, new30d] = await Promise.all([
            kitGet("/subscribers", { per_page: "1" }),
            kitGet("/subscribers", { per_page: "1", status: "active" }),
            kitGet("/subscribers", { per_page: "1", status: "cancelled" }),
            kitGet("/subscribers", { per_page: "1", created_after: daysAgoISO(7) }),
            kitGet("/subscribers", { per_page: "1", created_after: daysAgoISO(30) }),
        ]);
...
server.tool("list_subscribers_by_segment", "List subscribers filtered by tag, status, with pagination", {
    ...
    try {
        ...
        const data = await kitGet("/subscribers", params);
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
    catch (error) {
        return { content: [{ type: "text", text: `Error: ${error.message}` }], isError: true };
...
server.tool("get_sequence_performance", "Get sequence stats. If no sequence_id, lists all sequences; otherwise returns detail for one.", {
    sequence_id: z.string().optional().describe("Specific sequence ID to get details for"),
}, async ({ sequence_id }) => {
    try {
        if (sequence_id) {
            const data = await kitGet(`/sequences/${sequence_id}`);
            return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
        }
        const data = await kitGet("/sequences");
        return { content: [{ type: "text", text: JSON.stringify(data, null, 2) }] };
    }
    catch (error) {
        return { content: [{ type: "text", text: `Error: ${error.message}` }], isError: true };
...
server.tool("list_broadcasts", "List completed broadcasts with open/click/unsubscribe rates", {
    limit: z.number().optional().default(10).describe("Number of broadcasts to return (default 10)"),
}, async ({ limit }) => {
    try {
        const data = await kitGet("/broadcasts/stats");
...
server.tool("get_form_performance", "List all forms with subscriber counts", {}, async () => {
    try {
        const data = await kitGet("/forms");
...
server.tool("get_tag_health", "List all tags sorted by subscriber count descending", {}, async () => {
    try {
        const data = await kitGet("/tags");
...
server.tool("get_subscriber_growth", "Get subscriber growth over a period, broken into weekly buckets", {
    ...
    try {
        ...
        const totalNew = await kitGet("/subscribers", {
            per_page: "1",
        ...
        for (let i = 0; i < numWeeks; i++) {
            ...
            bucketPromises.push(kitGet("/subscribers", {
                per_page: "1",
