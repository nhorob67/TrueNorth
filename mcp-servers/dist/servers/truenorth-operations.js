...
// ── list_blockers ───────────────────────────────────────────────────
server.tool("list_blockers", "List blockers for an organization, optionally filtered by resolution state or severity", {
    ...
    let query = supabase
        .from("blockers")
        .select("id, description, severity, linked_entity_id, linked_entity_type, resolution_state, owner_id, resolved_at, resolution_notes, created_at, updated_at")
...
// ── list_decisions ──────────────────────────────────────────────────
server.tool("list_decisions", "List decisions for an organization, optionally filtered to pending-only or since a date", {
    org_id: z.string().describe("Organization UUID"),
    ...
    let query = supabase
        .from("decisions")
        .select("id, title, context, options_considered, final_decision, owner_id, linked_entity_id, linked_entity_type, decided_at, created_at")
        .eq("organization_id", org_id)
        .order("created_at", { ascending: false })
        .limit(limit ?? 30);
    if (pending_only)
        query = query.is("decided_at", null);
    if (since)
...
// ── list_commitments ────────────────────────────────────────────────
server.tool("list_commitments", "List commitments for an organization, optionally filtered by status or overdue-only", {
    org_id: z.string().describe("Organization UUID"),
    status: z
    ...
    let query = supabase
        .from("commitments")
        .select("id, description, owner_id, due_date, linked_entity_id, linked_entity_type, status, created_in, created_at, updated_at")
        .eq("organization_id", org_id)
...
// ── list_pulses ─────────────────────────────────────────────────────
server.tool("list_pulses", "List pulse check-ins for an organization, optionally filtered by date or user", {
    org_id: z.string().describe("Organization UUID"),
...
// ── get_pulse_summary ───────────────────────────────────────────────
server.tool("get_pulse_summary", "Aggregate pulse data over a date range — total pulses, unique contributors, items by type, top shipped, and recurring blocker themes", {
    org_id: z.string().describe("Organization UUID"),
    ...
    const itemsByType = { shipped: 0, blocker: 0, signal: 0 };
    const shippedTexts = [];
    const blockerTexts = [];
    for (const pulse of pulses) {
        ...
        for (const item of items) {
            ...
            if (item.type === "shipped")
                shippedTexts.push(item.text);
            if (item.type === "blocker")
                blockerTexts.push(item.text);
        }
    }
    const summary = {
...
server.tool("get_operating_health", "Get the latest operating health snapshot for a venture", {
    org_id: z.string().describe("Organization UUID"),
    ...
    return { content: [{ type: "text", text: JSON.stringify(data?.[0] ?? null, null, 2) }] };
});
// ── list_meeting_logs ───────────────────────────────────────────────
server.tool("list_meeting_logs", "List meeting logs for an organization, optionally filtered by venture", {
    org_id: z.string().describe("Organization UUID"),
    ...
    let query = supabase
        .from("meeting_logs")
        .select("id, meeting_type, started_at, completed_at, duration_seconds, output, facilitator_id, created_at")
        .eq("organization_id", org_id)
