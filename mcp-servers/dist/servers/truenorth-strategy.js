...
// ── get_vision ───────────────────────────────────────────────────────
server.tool("get_vision", "Get the venture's vision: BHAG, strategic filters, annual outcomes, and not-doing list", {
...
// ── list_bets ────────────────────────────────────────────────────────
server.tool("list_bets", "List bets for an organization, optionally filtered by venture or lifecycle status", {
    org_id: z.string().describe("Organization UUID"),
...
server.tool("get_bet_context", "Get enriched context for a single bet: linked KPIs, move velocity, blocker density, rhythm compliance. Essential for kill-switch assessments.", {
    org_id: z.string().describe("Organization UUID"),
    ...
    const { data: bet, error: betErr } = await supabase
        ...
        .single();
    if (betErr) {
        return { content: [{ type: "text", text: `Error: ${betErr.message}` }], isError: true };
    }
    ...
    const moveBlockerIds = new Set(allBlockers.filter((b) => b.linked_entity_id && moveIds.has(b.linked_entity_id)).map((b) => b.linked_entity_id));
    // Milestones
    ...
    const recentShipped = milestones.filter((m) => m.lifecycle_status === "shipped" && m.updated_at && m.updated_at >= fourteenDaysAgo);
    // Recurring moves
    ...
    let kpiTrendVsMoveExecution = "insufficient_data";
...
// ── list_moves ───────────────────────────────────────────────────────
server.tool("list_moves", "List moves, optionally filtered by venture, bet, type, or lifecycle status", {
    org_id: z.string().describe("Organization UUID"),
    venture_id: z.string().optional().describe("Filter by venture UUID"),
...
// ── list_blockers ────────────────────────────────────────────────────
server.tool("list_blockers", "List blockers, optionally filtered by resolution state or severity", {
    org_id: z.string().describe("Organization UUID"),
    resolution_state: z
...
// ── list_ideas ───────────────────────────────────────────────────────
server.tool("list_ideas", "List ideas with their filter results, optionally filtered by venture or lifecycle status", {
    org_id: z.string().describe("Organization UUID"),
    venture_id: z.string().optional().describe("Filter by venture UUID"),
    lifecycle_status: z
        .string()
