...
// ── log_action ──────────────────────────────────────────────────────
server.tool("log_action", "Log an AI action into the ai_actions table", {
    ...
    return { content: [{ type: "text", text: JSON.stringify({ action_id: data.id }, null, 2) }] };
});
// ── record_outcome ──────────────────────────────────────────────────
server.tool("record_outcome", "Update an existing ai_action's outcome", {
    ...
    return { content: [{ type: "text", text: JSON.stringify({ success: true, action_id }, null, 2) }] };
});
// ── list_pending_reviews ────────────────────────────────────────────
server.tool("list_pending_reviews", "List ai_actions where outcome is pending", {
    org_id: z.string().describe("Organization UUID"),
    ...
    let query = supabase
        ...
        .limit(limit ?? 20);
    if (agent_category)
        query = query.eq("agent_category", agent_category);
...
// ── get_trust_metrics ───────────────────────────────────────────────
server.tool("get_trust_metrics", "Aggregate ai_actions to compute acceptance/override rates", {
    org_id: z.string().describe("Organization UUID"),
    agent_category: z.string().optional().describe("Filter by agent category"),
...
server.tool("submit_reviewable_action", "Insert an ai_action with outcome=pending — a central write primitive for agents proposing changes", {
    org_id: z.string().describe("Organization UUID"),
...
server.tool("request_notification", "Send a notification to a specific user or all org admins", {
    org_id: z.string().describe("Organization UUID"),
    ...
    if (user_id) {
        ...
        const { data: members, error: membersErr } = await supabase
            ...
            .eq("role", "admin");
        if (membersErr) {
            return { content: [{ type: "text", text: `Error: ${membersErr.message}` }], isError: true };
        }
    ...
    return { content: [{ type: "text", text: JSON.stringify({ sent: userIds.length }, null, 2) }] };
});
// ── save_memory ─────────────────────────────────────────────────────
server.tool("save_memory", "Upsert a memory entry into the agent_memory table", {
    org_id: z.string().describe("Organization UUID"),
    ...
    return { content: [{ type: "text", text: JSON.stringify({ success: true, key, memory_type }, null, 2) }] };
});
// ── get_memory ──────────────────────────────────────────────────────
server.tool("get_memory", "Query memory entries from the agent_memory table", {
    agent_id: z.string().describe("Agent UUID"),
