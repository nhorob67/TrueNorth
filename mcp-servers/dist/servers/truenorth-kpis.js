...
// ── list_kpis ────────────────────────────────────────────────────────
server.tool("list_kpis", "List KPIs for an organization, optionally filtered by venture, health status, or lifecycle status", {
    ...
    let query = supabase
        .from("kpis")
        .select("id, name, description, unit, frequency, tier, directionality, owner_id, target, current_value, health_status, lifecycle_status, threshold_logic, action_playbook, formula_description, icon, created_at, updated_at")
...
// ── get_kpi ──────────────────────────────────────────────────────────
server.tool("get_kpi", "Get a single KPI with its thresholds, action playbook, and linked driver KPIs", {
    org_id: z.string().describe("Organization UUID"),
    kpi_id: z.string().describe("KPI UUID"),
}, async ({ org_id, kpi_id }) => {
    const supabase = getSupabase();
    const [{ data: kpi, error: kpiErr }, { data: drivers }] = await Promise.all([
        supabase
            .from("kpis")
            .select("*")
            .eq("id", kpi_id)
    ...
    if (kpiErr) {
        return { content: [{ type: "text", text: `Error: ${kpiErr.message}` }], isError: true };
    }
    const result = {
        ...kpi,
        drivers: (drivers ?? []).map((d) => d.kpis),
    };
    return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
});
// ── list_kpi_entries ─────────────────────────────────────────────────
server.tool("list_kpi_entries", "List time-series entries for a KPI, ordered by most recent first", {
    kpi_id: z.string().describe("KPI UUID"),
...
server.tool("get_kpi_health_summary", "Get an aggregated health summary: counts by status, plus the KPIs that changed health most recently", {
    org_id: z.string().describe("Organization UUID"),
    venture_id: z
        .string()
        .optional()
...
server.tool("get_bet_kpi_context", "Get KPI health data for all active bets in a venture — used by kill-switch for bet assessments", {
    org_id: z.string().describe("Organization UUID"),
    ...
    const { data: bets, error: betsErr } = await supabase
        ...
        .eq("lifecycle_status", "active");
    if (betsErr) {
        return { content: [{ type: "text", text: `Error: ${betsErr.message}` }], isError: true };
    }
    if (!bets || bets.length === 0) {
        return { content: [{ type: "text", text: JSON.stringify({ bets: [], message: "No active bets found" }) }] };
    }
