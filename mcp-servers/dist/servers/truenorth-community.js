 ...
 */
async function runDataExplorerSQL(sql) {
    const config = getDiscourseConfig();
    const headers = {
        "Api-Key": config.apiKey,
        "Api-Username": config.apiUsername,
    ...
    const createRes = await fetch(`${config.baseUrl}/admin/plugins/discourse-data-explorer/queries`, {
        method: "POST",
        headers,
        body: JSON.stringify({
            query: {
    ...
    try {
        const runRes = await fetch(`${config.baseUrl}/admin/plugins/discourse-data-explorer/queries/${queryId}/run`, { method: "POST", headers });
        if (!runRes.ok) {
        ...
        fetch(`${config.baseUrl}/admin/plugins/discourse-data-explorer/queries/${queryId}`, { method: "DELETE", headers }).catch(() => { });
    }
}
// ── get_community_health ────────────────────────────────────────────
server.tool("get_community_health", "Get aggregate community health metrics: member counts, activity levels, engagement rates, and trends", {}, async () => {
    const about = await discourseGet("/about.json");
    const stats = about.about.stats;
    const wauOverMau = stats.active_users_30_days === 0
        ? 0
        : Math.round((stats.active_users_7_days / stats.active_users_30_days) * 10000) / 100;
    // Fetch DAU from admin dashboard
    let dau = 0;
    try {
        const dashboard = await discourseGet("/admin/dashboard.json");
        const dauReport = dashboard.global_reports?.find((r) => r.type === "daily_engaged_users");
...
server.tool("list_at_risk_members", "Find members with declining activity — those who were active in prior weeks but have gone quiet recently. Useful for churn prevention outreach.", {
    ...
    const sql = `
      ...
      LIMIT ${maxResults}
    `;
    const result = await runDataExplorerSQL(sql);
...
// ── list_churn_events ───────────────────────────────────────────────
server.tool("list_churn_events", "List members who were recently active but have not visited in a long time — likely churned. Shows their last activity for pattern detection.", {
    min_days_absent: z
        .number()
        .optional()
...
// ── get_member_activity ─────────────────────────────────────────────
server.tool("get_member_activity", "Get detailed activity metrics for a specific member by username", {
    username: z.string().describe("Discourse username"),
}, async ({ username }) => {
    // Fetch user summary (public endpoint)
    const summary = await discourseGet(`/u/${encodeURIComponent(username)}/summary.json`);
    // Fetch user profile for dates
    const profile = await discourseGet(`/u/${encodeURIComponent(username)}.json`);
...
server.tool("get_engagement_report", "Get a weekly engagement breakdown: new members, top contributors, posts with high reply counts, and content gaps (unanswered topics)", {
    ...
    const [topContrib, unanswered, newMembers, hotTopics] = await Promise.all([
        runDataExplorerSQL(topContribSql),
        runDataExplorerSQL(unansweredSql),
        runDataExplorerSQL(newMembersSql),
        runDataExplorerSQL(hotTopicsSql),
    ]);
    const mapRows = (res) => (res.rows ?? []).map((row) => {
        const obj = {};
...
// ── get_cohort_metrics ──────────────────────────────────────────────
server.tool("get_cohort_metrics", "Get member activity breakdown by trust level cohort — shows how members at each trust level engage differently", {}, async () => {
    const sql = `
      SELECT
        u.trust_level,
...
server.tool("get_new_member_activation", "Find new members and their activation status — highlights members who joined recently but haven't posted, for outreach within the critical first 7 days", {
    ...
    return {
        content: [
            { type: "text", text: JSON.stringify({ summary, members }, null, 2) },
        ],
    };
});
// ── get_activation_funnel ───────────────────────────────────────────
server.tool("get_activation_funnel", "Get the new member activation funnel: joined → viewed topics → first post → second post. Shows where new members drop off.", {
    ...
    if (!row) {
        return {
            content: [
                { type: "text", text: JSON.stringify({ period_days: lookback, total_new_members: 0 }, null, 2) },
            ],
        };
    }
    const data = {};
    ...
    const total = data.total_new_members || 1;
    const funnel = {
        period_days: lookback,
        ...data,
        funnel_rates: {
            view_rate_pct: Math.round((data.viewed_topics / total) * 100),
            first_post_pct: Math.round((data.made_first_post / total) * 100),
            second_post_pct: Math.round((data.made_second_post / total) * 100),
            five_posts_pct: Math.round((data.made_five_posts / total) * 100),
            liked_pct: Math.round((data.gave_a_like / total) * 100),
        },
    };
