...
function getStripeConfig() {
  ...
  if (!apiKey) throw new Error("Missing STRIPE_SECRET_KEY env var");
  return { apiKey };
}
...
async function stripeList<T extends { id: string }>(path: string, params?: Record<string, string>): Promise<T[]> {
  const all: T[] = [];
  let startingAfter: string | undefined;
  while (true) {
    const p = new URLSearchParams({ limit: "100", ...params });
    if (startingAfter) p.set("starting_after", startingAfter);
    const res = await stripeGet<{ data: T[]; has_more: boolean }>(`${path}?${p.toString()}`);
    all.push(...res.data);
...
server.tool(
  "get_mrr_summary",
  "Get current MRR breakdown by product, plus net-new and churn MRR over the last 30 days",
  {},
  async () => {
    try {
      ...
      const activeSubs = await stripeList<any>("/subscriptions", { status: "active" });

      const byProduct: Record<string, { mrr: number; count: number }> = {};
      let totalMrr = 0;
      ...
      for (const sub of activeSubs) {
        for (const item of sub.items?.data ?? []) {
          ...
          if (interval === "year") monthlyAmount = Math.round(amount / 12);
          else if (interval === "week") monthlyAmount = Math.round(amount * 52 / 12);
          else if (interval === "day") monthlyAmount = Math.round(amount * 365 / 12);

          const name = item.plan?.nickname || item.plan?.product || "Unknown";
          if (!byProduct[name]) byProduct[name] = { mrr: 0, count: 0 };
      ...
      const canceledSubs = await stripeList<any>("/subscriptions", { status: "canceled" });
      let churnMrr = 0;
...
server.tool(
  "list_subscription_events",
  "List recent subscription-related events (creates, updates, cancellations)",
  {
  ...
  async ({ days, limit }) => {
    try {
      ...
      const res = await stripeGet<{ data: any[] }>(
        `/events?created[gte]=${since}&limit=100`
      );

      const subEvents = res.data
        .filter((e: any) => e.type.includes("customer.subscription"))
        .slice(0, maxResults);

      const result = subEvents.map((e: any) => {
...
server.tool(
  "get_churn_analysis",
  "Analyze recently canceled subscriptions with duration and cancel reason",
  {
  ...
  async ({ days }) => {
    try {
      ...
      const recent = canceledSubs.filter(
        (sub: any) => sub.canceled_at && sub.canceled_at >= since
      );

      const result = recent.map((sub: any) => {
...
server.tool(
  "list_failed_charges",
  "List recent failed or incomplete charges for dunning and recovery analysis",
  {
    days: z.number().optional().describe("Lookback window in days (default: 7)"),
...
server.tool(
  "get_revenue_by_product",
  "Break down revenue by product from paid invoices over a given period",
  {
  ...
  async ({ months }) => {
    try {
      ...
      const invoices = await stripeList<any>("/invoices", {
        status: "paid",
        "created[gte]": String(since),
      });

      const byProduct: Record<string, { revenue_cents: number; invoice_count: number }> = {};
      let totalRevenue = 0;
      ...
      for (const inv of invoices) {
        for (const line of inv.lines?.data ?? []) {
          ...
          if (!byProduct[name]) byProduct[name] = { revenue_cents: 0, invoice_count: 0 };
...
server.tool(
  "get_customer_cohort",
  "Cohort analysis showing signups per month and their retention rates",
  {
  ...
  async ({ months }) => {
    try {
      ...
      const subs = await stripeList<any>("/subscriptions", {
        status: "all",
        "created[gte]": String(since),
      });
      ...
      for (const sub of subs) {
        ...
        if (sub.status === "active") cohorts[month].active += 1;
      }
