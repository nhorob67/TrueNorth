...
From the repo root, `npx playwright test e2e/sidebar-links.spec.ts` runs an end-to-end spec.
...
Do not commit `.env*` files or secrets. Start local setup from `app/.env.local.example`, and keep API keys, service-role credentials, and test credentials out of source changes.
